          ...
          {canManage && (
            <div className="mt-5">
              ...
              {myRole === 'owner' && (
                <button
                  onClick={() => {
                    if (confirm(`Permanently delete ${member.full_name}? This will remove all their messages, reactions, and account. This cannot be undone.`)) {
                      handleAction('permanent_delete')
                    }
                  }}
                  disabled={actionLoading !== null}
                  className="w-full mt-2 px-3 py-2 rounded-xl text-[11px] font-bold text-red-400/50 hover:text-red-400 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all disabled:opacity-50"
                >
                  {actionLoading === 'permanent_delete' ? (
                    <span className="flex items-center justify-center gap-2">
                      <span className="w-3 h-3 border-2 border-red-400/20 border-t-red-400/60 rounded-full animate-spin" />
                      Deleting...
                    </span>
